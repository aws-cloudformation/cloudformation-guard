...
use crate::path_value::{PathAwareValue, QueryResolver};
use crate::values::*;
use crate::{Evaluate, EvaluationContext, EvaluationType, Result, Status};
...
impl<'loc, T: Evaluate + 'loc> Evaluate for Conjunctions<T> {
    ...
    fn evaluate<'s>(
        ...
        Ok(loop {
            ...
            let mut num_fails = 0;
            let is_guard_clause =
                std::any::type_name::<T>() == std::any::type_name::<GuardClause>();
            'conjunction: for conjunction in self {
                let mut num_of_disjunction_fails = 0;
                let mut report = if is_guard_clause {
                    Some(AutoReport::new(
...
impl<'s, 'loc> RootScope<'s, 'loc> {
    pub fn new(rules: &'s RulesFile<'loc>, value: &'s PathAwareValue) -> Result<Self> {
...
#[cfg(test)]
#[path = "evaluate_scope_tests.rs"]
mod evaluate_scope_tests;
