...
impl<'value> Node<'value> {
    ...
    pub fn value(&self) -> &PathAwareValue {
        self.value
    }
}

#[derive(Clone, Debug, PartialEq)]
pub enum TraversalResult<'a, 'b> {
    Value(&'b Node<'a>),
    Key(&'a str),
}
...
impl<'a, 'b> TraversalResult<'a, 'b> {
    pub fn as_value(&self) -> Option<&Node<'_>> {
        match self {
            Self::Value(n) => Some(n),
            _ => None,
        }
...
impl<'value> Traversal<'value> {
    ...
    pub fn at<'traverse>(
        ...
        pointer: &str,
        node: &'traverse Node,
    ) -> crate::Result<TraversalResult<'traverse, 'traverse>> {
