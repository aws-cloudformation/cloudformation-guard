...
use crate::{CmpOperator, QueryResult, UnResolved};

#[derive(Clone, Debug)]
pub struct LhsRhsPair {
    pub lhs: Rc<PathAwareValue>,
    pub rhs: Rc<PathAwareValue>,
}
...
#[derive(Clone, Debug)]
pub struct QueryIn {
    pub diff: Vec<Rc<PathAwareValue>>,
    pub lhs: Vec<Rc<PathAwareValue>>,
    pub rhs: Vec<Rc<PathAwareValue>>,
}
...
#[derive(Clone, Debug)]
pub enum ValueEvalResult {
    LhsUnresolved(UnResolved),
    ComparisonResult(ComparisonResult),
}

impl ValueEvalResult {
    pub fn fail<C>(self, c: C) -> ValueEvalResult
    where
...
pub trait Comparator {
    fn compare(&self, lhs: &[QueryResult], rhs: &[QueryResult]) -> crate::Result<EvalResult>;
}

#[allow(dead_code)]
pub trait UnaryComparator {
    fn compare(&self, lhs: &[QueryResult]) -> crate::Result<EvalResult>;
}

struct CommonOperator {
    comparator: fn(&PathAwareValue, &PathAwareValue) -> crate::Result<bool>,
}
...
where
    C: Fn(&PathAwareValue, &PathAwareValue) -> crate::Result<bool>,
{
...
impl Comparator for crate::CmpOperator {
    fn compare<'value>(
        &self,
        lhs: &[QueryResult],
        rhs: &[QueryResult],
        ...
        match self {
            ...
            .compare(lhs, rhs),
            _ => Err(crate::Error::IncompatibleError(format!(
...
impl Comparator for (crate::CmpOperator, bool) {
    fn compare<'value>(
        &self,
        lhs: &[QueryResult],
        rhs: &[QueryResult],
                                    ...
                                    match c {
                                        Compare::QueryIn(qin) => {
                                            let reverse_diff = if rhs.len() >= lhs.len()
                                                && matches!(self.0, crate::CmpOperator::Eq)
                                            {
