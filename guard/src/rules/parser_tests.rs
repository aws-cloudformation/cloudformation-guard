...
fn test_access() {
    ...
    let expectations = [
        ...
        Ok((
            ...
            AccessQuery {
                query: vec![QueryPart::Key("%engine".to_string()), QueryPart::AllValues],
                match_all: true,
            },
        )),
        // matches { 'engine': [{'type': 'cfn', 'position': 1, 'other': 20}, {'type': 'tf', 'position': 2, 'other': 10}] }
        ...
        Ok((
            ...
            AccessQuery {
                query: vec![
                    QueryPart::Key("engine".to_string()),
                    QueryPart::Filter(vec![vec![GuardClause::Clause(GuardAccessClause {
...
fn test_clause_success() {
    ...
    for each_lhs in lhs.iter() {
        ...
        let _lhs_access = testing_access_with_cmp(
            &separators,
            &comparators,
            *each_lhs,
            rhs,
        ...
        Ok((
            ...
            AccessQuery {
                query: vec![
                    ...
                    QueryPart::AllValues,
                    QueryPart::Filter(Conjunctions::from([Disjunctions::from([
                        GuardClause::Clause(GuardAccessClause {
                            access_clause: AccessClause {
                                compare_with: Some(LetValue::Value(Value::Regex(
                                    "AWS::RDS".to_string(),
                                ))),
                                comparator: (CmpOperator::Eq, false),
                                query: AccessQuery {
                    ...
                    QueryPart::Filter(Conjunctions::from([
                        ...
                        Disjunctions::from([GuardClause::Clause(GuardAccessClause {
                            access_clause: AccessClause {
                                ...
                                query: AccessQuery {
                                    query: vec![QueryPart::Key(String::from("deletion_policy"))],
                                    match_all: true,
                                },
                                custom_message: None,
                                location: FileLocation {
                        ...
                        Disjunctions::from([GuardClause::Clause(GuardAccessClause {
                            access_clause: AccessClause {
                                compare_with: Some(LetValue::Value(Value::String(
                                    "RETAIN".to_string(),
                                ))),
                                comparator: (CmpOperator::Eq, false),
                                query: AccessQuery {
...
fn test_assignments() {
    ...
    let expectations = [
        ...
        Ok((
            ...
            LetExpr {
                var: String::from("x"),
                value: LetValue::Value(Value::List(vec![Value::Int(10), Value::Int(20)])),
            },
        )),
        // "let x = engine",       // 4 Ok
        Ok((
            ...
            TypeBlock {
                ...
                conditions: Some(vec![vec![WhenGuardClause::Clause(GuardAccessClause {
                    access_clause: AccessClause {
                        ...
                        compare_with: Some(LetValue::Value(Value::String(String::from(
                            "m4.xlarge",
                        )))),
                        custom_message: None,
                    },
                        ...
                        conjunctions: Conjunctions::from([Disjunctions::from([
                            GuardClause::Clause(GuardAccessClause {
                                ...
                                access_clause: AccessClause {
                                    ...
                                    comparator: (CmpOperator::In, false),
                                    compare_with: Some(LetValue::Value(Value::List(vec![
                                        Value::String(String::from("ExternalS3Approved")),
                                    ]))),
                                    custom_message: None,
                                    location: FileLocation {
                                        file_name: "",
...
fn select_any_one_from_list_clauses() -> Result<(), Error> {
    ...
    let expected = GuardClause::Clause(GuardAccessClause {
        access_clause: AccessClause {
            ...
            compare_with: Some(LetValue::Value(Value::Regex(
                "\\{\\{resolve:secretsmanager".to_string(),
            ))),
            comparator: (CmpOperator::Eq, false),
            custom_message: None,
            query: AccessQuery {
        ...
        access_clause: AccessClause {
            query: AccessQuery {
                ...
                query: vec![
                    ...
                    QueryPart::MapKeyFilter(MapKeyFilterClause {
                        comparator: (CmpOperator::Eq, false),
                        compare_with: LetValue::Value(Value::Regex(
            ...
            QueryPart::Filter(Conjunctions::from([Disjunctions::from([
                GuardClause::Clause(GuardAccessClause {
                    ...
                    access_clause: AccessClause {
                        ...
                        compare_with: Some(LetValue::Value(Value::Map(make_linked_hashmap(vec![
