...
fn test_assignment() {
    ...
    let expected = vec![
        Ok((
            make_empty_span(examples[0].len()),
            Assignment {
        ...
        Err(nom::Err::Error(ParserError {
            ...
            span: unsafe {
                Span::new_from_raw_offset(
                    examples[1].find("propertyaccess").unwrap(),
                    1,
                    examples[1].trim_start_matches("let"),
                    "",
                )
            },
        ...
        Err(nom::Err::Failure(ParserError {
            ...
            span: unsafe {
                Span::new_from_raw_offset(
                    examples[2].find("IN").unwrap(),
                    1,
                    examples[2].trim_start_matches("let my "),
                    "",
                )
            },
    ...
    for (i, example) in examples.into_iter().enumerate() {
        assert_eq!(assignment(from_str2(example)), expected[i])
    }
}
...
fn test_parse_old_guard_values_integer() {
    ...
    let example_expected = vec![
        ...
        (
            ...
            OldGuardValues::Value(Value::String(String::from("This is a quoted string"))),
        ),
        (
            "  this is a bare string   ",
            OldGuardValues::Value(Value::String(String::from("this is a bare string"))),
        ),
        (
            "{ \"test\": \"key\", \"myValue\": { \"foo\": [ 1, 2, 3]}}",
            OldGuardValues::Value(Value::Map(map)),
        ),
        (
            "  /match(.*)this/",
            OldGuardValues::Value(Value::Regex("match(.*)this".to_string())),
        ),
        ("true", OldGuardValues::Value(Value::Bool(true))),
        (
            "[\"list\", \"of\", \"values\"]",
            OldGuardValues::Value(Value::List(string_list)),
        ),
        (
            "%deref_this",
            OldGuardValues::VariableAccess(String::from("deref_this")),
        ),
        // test version numbers correctly get parsed as strings and not as floats
        (
            "2.1.4-latest",
            OldGuardValues::Value(Value::String(String::from("2.1.4-latest"))),
        ),
    ];
...
fn test_value_cmp() {
    ...
    for (i, example) in example.into_iter().enumerate() {
        assert_eq!(value_operator(from_str2(example)), expected[i])
    }
}
...
fn test_property_path() {
    ...
    for (i, example) in examples.into_iter().enumerate() {
        assert_eq!(property_path(from_str2(example)), expectations[i])
    }
}
...
fn test_prop_comparison_invalid_operator_fail() {
    ...
    let error_span =
        unsafe { Span::new_from_raw_offset(example.find("NOT").unwrap(), 1, "NOT EXISTS", "") };
...
fn test_clause_with_message() {
    ...
    let conditional_rule = Rule::Conditional(ConditionalRule {
        type_name: TypeName {
            type_name: String::from("AWS::S3::Bucket"),
        },
        when_condition,
    ...
    let basic_rule_1 = Rule::Basic(BaseRule {
        type_name: TypeName {
    ...
    let basic_rule_2 = Rule::Basic(BaseRule {
        type_name: TypeName {
            type_name: String::from("AWS::S3::Bucket"),
        },
        property_comparison: PropertyComparison {
        ...
        custom_message: Some(String::from(
            "Buckets should be encrypted, or instance type large, or property path in a,b,c",
        )),
    });
    ...
    assert_eq!(parsed_rules, Ok((span, RuleLineType::Clause(clause))));
}
...
fn test_parse_rules_file() {
    ...
    let parsed_rules =
        parse_rules_file(&String::from(example), &String::from("file_name")).unwrap();
...
fn test_parse_rules_file_rule_error() {
    ...
    assert!(parse_rules_file(&String::from(example), &String::from("file_name")).is_err());
}
...
fn test_disjunction_basic_clauses() {
    ...
    let encryption_rule = Rule::Basic(BaseRule {
        type_name: TypeName {
            type_name: String::from("AWS::EC2::Volume"),
        },
        property_comparison: PropertyComparison {
    ...
    let volume_size_100 = Rule::Basic(BaseRule {
        type_name: TypeName {
            type_name: String::from("AWS::EC2::Volume"),
        },
        property_comparison: PropertyComparison {
    ...
    let volume_size_50 = Rule::Basic(BaseRule {
        type_name: TypeName {
            type_name: String::from("AWS::EC2::Volume"),
        },
        property_comparison: PropertyComparison {
...
fn test_disjunction_conditional_clauses() {
    ...
    let m2_conditional_rule = Rule::Conditional(ConditionalRule {
        type_name: TypeName {
            type_name: String::from("AWS::EC2::Instance"),
        },
    ...
    let t2_conditional_rule = Rule::Conditional(ConditionalRule {
        type_name: TypeName {
            type_name: String::from("AWS::EC2::Instance"),
        },
    ...
    let expected_rules = vec![RuleLineType::Clause(Clause {
